...
from os import getcwd, getenv, listdir, makedirs, name, path, rmdir, name, remove
...
class Downloader:
    ...
    def _finalize_download(self, file_info: dict[str, str], tmp_file: str, has_size: str) -> None:
        ...
        If the file is a RAR archive, extracts it and deletes the archive.
        ...
        if path.getsize(tmp_file) == int(has_size):
            ...
            _print(
                f"{TERMINAL_CLEAR_LINE}"
                f"Downloading {file_info['filename']}: {path.getsize(final_path)} "
                f"of {has_size} Done!{NEW_LINE}"
            )
            
            # Check if the file is a RAR archive and extract it
            filename_lower = file_info["filename"].lower()
            if filename_lower.endswith('.rar'):
                # Check if this is the first part of a multi-part archive or a single archive
                # For multi-part archives, we only extract when the .rar file is downloaded
                self._extract_rar(final_path, file_info["path"])


    def _extract_rar(self, rar_path: str, extract_dir: str) -> None:
        """
        _extract_rar

        Extracts a RAR archive and deletes it after extraction.

        :param rar_path: path to the RAR archive.
        :param extract_dir: directory where to extract the archive.
        :return:
        """

        _print(f"{TERMINAL_CLEAR_LINE}Extraction de {path.basename(rar_path)}...{NEW_LINE}")
        
        try:
            # Try using patoolib first (if available)
            try:
                import patoolib
                patoolib.extract_archive(rar_path, outdir=extract_dir)
                _print(f"{TERMINAL_CLEAR_LINE}Extraction terminée: {path.basename(rar_path)}{NEW_LINE}")
                remove(rar_path)
                _print(f"{TERMINAL_CLEAR_LINE}Archive supprimée: {path.basename(rar_path)}{NEW_LINE}")
                return
            except ImportError:
                pass
            except Exception as e:
                _print(f"{TERMINAL_CLEAR_LINE}Erreur avec patoolib: {e}{NEW_LINE}")
            ...
            try:
                ...
                _print(f"{TERMINAL_CLEAR_LINE}Erreur avec rarfile: {e}{NEW_LINE}")
            
            # Try using unrar command line tool (Windows)
            if name == "nt":
                # Common paths for unrar on Windows
                unrar_paths = [
                ...
                for unrar_exe in unrar_paths:
                    try:
                        result = subprocess.run(
                            [unrar_exe, "x", "-o+", rar_path, extract_dir],
                            capture_output=True,
                            text=True,
                            timeout=300
                        )
            ...
            for sevenz_exe in sevenz_paths:
                try:
                    result = subprocess.run(
                        [sevenz_exe, "x", rar_path, f"-o{extract_dir}", "-y"],
                        capture_output=True,
                        text=True,
                        timeout=300
                    )
            ...
            _print(f"{TERMINAL_CLEAR_LINE}Impossible d'extraire {path.basename(rar_path)}: aucun outil d'extraction RAR trouvé.{NEW_LINE}")
            _print(f"{TERMINAL_CLEAR_LINE}Veuillez installer WinRAR, 7-Zip ou Python rarfile/patoolib.{NEW_LINE}")
            
        except Exception as e:
            _print(f"{TERMINAL_CLEAR_LINE}Erreur lors de l'extraction de {path.basename(rar_path)}: {e}{NEW_LINE}")
...
class Manager:
    def __init__(self, url_or_file: str, password: str | None = None, game_name: str | None = None) -> None:
        """
        Manager
        ...
        :game_name: Name of the game (used to set download directory to c:\\sroff-game\\game_name).
        :return:
        """

        # If game_name is provided, use c:\sroff-game\game_name as root directory
        if game_name:
            root_dir = path.join("c:\\", "sroff-game", game_name)
        else:
            root_dir = getenv("GF_DOWNLOAD_DIR")
...
if __name__ == "__main__":
    ...
    if argc > 1:
        ...
        manager: Manager = Manager(url_or_file=url_or_file, password=password, game_name=game_name)
        ...
        die(f"Usage:"
            ...
            f"{NEW_LINE}"
            f"python gofile-downloader.py https://gofile.io/d/contentid password nomdujeux"
        )
